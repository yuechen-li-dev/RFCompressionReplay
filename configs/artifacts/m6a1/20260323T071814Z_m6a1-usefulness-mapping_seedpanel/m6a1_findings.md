...
- Retention mode used: `milestone` (with only manifest + M6a1 summary artifacts retained at the top level for this milestone)
- Config provenance: `m6a1-usefulness-mapping` / `m6a1`

## Task-Family Read

### `structured-burst-vs-noise-only`

- Best overall by median AUC: `cav` at 0.633 median AUC with 12/18 best-or-tied-best condition wins.
- Best baseline: `cav` at 0.633 median AUC.
- Best compression-derived detector: `lzmsa-paper` at 0.492 median AUC.
- RMS-normalized mean vs `lzmsa-paper`: normalized mean median AUC 0.488 vs paper 0.492; median pairwise AUC gap (normalized minus paper) 0.004.
...
- Best overall by median AUC: `ed` at 0.992 median AUC with 16/18 best-or-tied-best condition wins.
- Best baseline: `ed` at 0.992 median AUC.
- Best compression-derived detector: `lzmsa-paper` at 0.549 median AUC.
- RMS-normalized mean vs `lzmsa-paper`: normalized mean median AUC 0.476 vs paper 0.549; median pairwise AUC gap (normalized minus paper) -0.054.
...
- Best overall by median AUC: `cav` at 0.971 median AUC with 16/18 best-or-tied-best condition wins.
- Best baseline: `cav` at 0.971 median AUC.
- Best compression-derived detector: `lzmsa-rms-normalized-mean-compressed-byte-value` at 0.520 median AUC.
- RMS-normalized mean vs `lzmsa-paper`: normalized mean median AUC 0.520 vs paper 0.472; median pairwise AUC gap (normalized minus paper) 0.033.
- Cautious read: ED/CAV remained at least as strong on median AUC, while the normalized-mean proxy stayed close to lzmsa-paper as the simpler compression summary.
...
- Against the best baseline within each task family, its median AUC gap ranged from -0.470 to -0.149.
